...
#if defined(_SDCC_) || defined(__SDCC)
 /* Patched: allow ST SPL to compile with SDCC.
    _SDCC_ is expected to come from the build system (-D_SDCC_=1).
    We also self-define it here in case only __SDCC is set.        */
 #ifndef _SDCC_
  #define _SDCC_
 #endif
...
#define LSI_VALUE   ((uint32_t)38000)    /*!< Typical Value of the LSI in Hz */
 ...
 #define IN_RAM
#elif defined (_SDCC_)
 /* Patched: SDCC does not have far/near/tiny/eeprom qualifiers on STM8. */
 #define FAR
 #define NEAR
 #define TINY
 #define EEPROM
...
#elif defined(_SDCC_)
 /* Patched: SDCC inline-assembly syntax for STM8 instructions.  */
...
#ifdef _SDCC_
 /* SDCC uses  __interrupt(N)  for STM8 ISRs (N = irq number).           */
...
#elif defined(_SDCC_)
 #define INTERRUPT     /* SDCC uses __interrupt at declaration site */
#endif /* _COSMIC_ */
